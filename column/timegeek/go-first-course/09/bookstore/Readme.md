...
https://www.yuque.com/dreamoflife/vtuyms/uk71ln7igt2g4all
...
```
go get：用于下载和安装指定的Go包及其依赖项。它可以从远程存储库中获取包，并将其安装到Go工作区中。用于下载并安装包及其依赖项，更新 go.mod 文件。

主要特点
- 获取和安装包：下载指定的包及其所有依赖项，并安装到GOPATH的pkg和bin目录中。
- 添加依赖：如果当前目录是一个Go模块，则会更新go.mod文件以添加新的依赖项。
- 支持版本：可以指定要获取的包的特定版本。

go install：用于编译并安装指定的Go包。它会编译包，并将生成的可执行文件安装到GOPATH/bin目录中，或者模块模式下的GOBIN目录中。不更改 go.mod 文件。

主要特点
- 编译并安装：编译指定的包并将其安装到GOPATH/bin目录中。
- 安装可执行文件：主要用于安装可执行文件而不是库包。
- 支持模块模式：在模块模式下，go install 支持使用版本指定。
